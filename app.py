...
with tab_pipeline:
    ...
    if not st.session_state.selected_agents:
        ...
        if PLOTLY_AVAILABLE and st.session_state.pipeline_results:
            ...
            if fig:
                # BUG FIX: Added a unique key to prevent DuplicateElementId error
                st.plotly_chart(fig, use_container_width=True, key="pipeline_flow_chart")
...
with tab_dashboard:
    ...
    with col_viz2:
        ...
        if st.session_state.pipeline_results and PLOTLY_AVAILABLE:
            ...
            if fig:
                # BUG FIX: Added a unique key to prevent DuplicateElementId error
                st.plotly_chart(fig, use_container_width=True, key="dashboard_pipeline_flow_chart")
        else:
...
chevron_right
Of course. I will enhance the application to allow users to choose their preferred Optical Character Recognition (OCR) method for processing PDF documents. You'll be able to select between a fast, standard Python-based library and a more powerful but slower LLM-based vision model (like GPT-4o or Gemini), which is ideal for scanned documents or complex layouts.
Here are the key improvements in the updated code:
OCR Method Selection: In the "Upload & Parse" tab, a new "OCR & Parsing Settings" section allows you to choose between "Standard (Python-based)" and "Advanced (LLM-based)" OCR.
LLM Provider & Model Choice: When you select "Advanced OCR," you can specify which LLM provider (Gemini or OpenAI) and which vision model (e.g., gemini-1.5-pro-latest, gpt-4o) you want to use.
New Dependencies: This feature requires the pdf2image library and a system-level dependency called poppler. The application will show an informational message about these requirements.
Enhanced User Feedback: While processing with an LLM, a detailed progress bar will show the status page-by-page, so you know it's working.
Below is the complete, improved code with the new OCR selection functionality integrated.
code
Python
import os
import io
...
with tab_agent_select:
    ...
    if not all_agents:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.download_button("📥 Download Sample agents.yaml", data=DEFAULT_SAMPLE_AGENTS.encode('utf-8'), file_name="agents_sample.yaml", mime="text/yaml", use_container_width=True)
        ...
        if agents_file:
            try:
                ...
                if isinstance(uploaded_config, dict) and "agents" in uploaded_config and isinstance(uploaded_config["agents"], list):
                    ...
                    st.error("Invalid YAML structure. Must contain a top-level 'agents' list.")
            except Exception as e:
                st.error(f"Error loading file: {e}")
    else:
        st.subheader(f"🎯 {t('select_agents')}")
        ...
        for i, agent in enumerate(all_agents):
            ...
            col1, col2 = st.columns([1, 4])
            with col1:
                is_selected = st.checkbox(" ", value=agent_id in st.session_state.selected_agents, key=f"select_{agent_id}_{i}")
                if is_selected: selected_agent_ids.append(agent_id)
            ...
            with col2:
                with st.expander(f"{'✅' if is_selected else '⬜'} {agent_name}", expanded=False):
                    ...
                    provider = c1.selectbox("Provider", ["auto", "gemini", "openai", "grok"], index=["auto", "gemini", "openai", "grok"].index(model_cfg.get("provider", "auto")), key=f"provider_{agent_id}_{i}")
                    model_name = c2.text_input("Model", value=model_cfg.get("name", ""), key=f"model_{agent_id}_{i}")
                    temperature = c3.slider("Temperature", 0.0, 1.0, float(model_cfg.get("temperature", 0.3)), 0.05, key=f"temp_{agent_id}_{i}")
        ...
        st.session_state.selected_agents = selected_agent_ids
        if selected_agent_ids:
            st.success(f"✅ Selected {len(selected_agent_ids)} agents for the pipeline.")
